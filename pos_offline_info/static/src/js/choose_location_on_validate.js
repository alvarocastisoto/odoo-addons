...
/* estado temporal: línea (cid) -> location_id */
const SRC_LOC_BY_CID = new Map();

/* ===== helper: consigue filas 'where' desde cache o servidor ===== */
...
/* ===== extiende Orderline para arrastrar pos_src_location_id ===== */
...
/* ===== servicio: inyecta id de ubicación antes de enviar el pedido ===== */
registry.category("services").add("pos_choose_location_on_validate", {
  ...
  start(env) {
    ...
    /* patch de validateOrder: pide ubicación por línea usando where (cache o RPC) */
      ...
      async validateOrder(isForceValidate) {
        ...
        if (order?.get_orderlines) {
          for (const line of order.get_orderlines()) {
            ...
            /* NEW: saca where desde cache o servidor */
            const rows = await getWhereRows(env, prod.id);

            /* agrupa por location_id y prepara opciones */
            const map = new Map();
            for (const r of rows) {
              const id = locId(r);
              if (!id) continue;
