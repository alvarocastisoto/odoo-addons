...
class ProductProduct(models.Model):
    ...
    def pos_where(self, product_id, config_id):
        """Wrapper simple para compatibilidad con el front:
    ...
    def pos_where_bulk(self, product_ids, config_id):
        ...
        whs = (
            self.env["stock.warehouse"].sudo().search([("company_id", "=", company_id)])
        )
        wh_map = {
            w.lot_stock_id.id: (
        ...
        _logger.info(
            "pos_where_bulk: products=%s rows(nonempty)=%s",
            len(products),
            sum(bool(v) for v in rows_by_product.values()),
        )
